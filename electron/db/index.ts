...
class JsonDatabase {
  ...
  async replaceDocument(id: string, filePath: string) {
    await this.init();
    return this.withLock(async () => {
      const idx = this.state!.documents.findIndex((d) => d.id === id);
      if (idx === -1) throw new Error('Dokument ne obstaja');

      const oldDoc = this.state!.documents[idx];
      // Remove old file if it exists
      if (fs.existsSync(oldDoc.stored_path)) {
        fs.unlinkSync(oldDoc.stored_path);
      }
  ...
  async importPhasesCsv(csv: string, projectId?: string) {
    ...
    return this.withLock(async () => {
      ...
      const mainPhaseIdHints = new Map<string, number>();
      ...
      const sortedPhases = [...projectPhases].sort((a, b) => {
        ...
        if (idHintA !== undefined && idHintB !== undefined) return idHintA - idHintB;
        if (idHintA !== undefined) return -1;
        if (idHintB !== undefined) return 1;
